...
pane_exists() {
	...
	local pane_index="$3"
	$TMUX_PROGRAM list-panes -t "${session_name}:${window_number}" -F "#{pane_index}" 2>/dev/null |
		\grep -q "^$pane_index$"
}
...
window_exists() {
	local session_name="$1"
	local window_number="$2"
	$TMUX_PROGRAM list-windows -t "$session_name" -F "#{window_index}" 2>/dev/null |
		\grep -q "^$window_number$"
}

session_exists() {
	local session_name="$1"
	$TMUX_PROGRAM has-session -t "$session_name" 2>/dev/null
}

first_window_num() {
	$TMUX_PROGRAM show -gv base-index
}
...
new_window() {
	...
	if is_restoring_pane_contents && pane_contents_file_exists "$pane_id"; then
		...
		$TMUX_PROGRAM new-window -d -t "${session_name}:${window_number}" -c "$dir" "$pane_creation_command"
	else
		$TMUX_PROGRAM new-window -d -t "${session_name}:${window_number}" -c "$dir"
	fi
}
...
new_session() {
	...
	if is_restoring_pane_contents && pane_contents_file_exists "$pane_id"; then
		local pane_creation_command="$(pane_creation_command "$session_name" "$window_number" "$pane_index")"
		TMUX="" $TMUX_PROGRAM -S "$(tmux_socket)" new-session -d -s "$session_name" -c "$dir" "$pane_creation_command"
	else
		TMUX="" $TMUX_PROGRAM -S "$(tmux_socket)" new-session -d -s "$session_name" -c "$dir"
	fi
	# change first window number if necessary
	local created_window_num="$(first_window_num)"
	if [ $created_window_num -ne $window_number ]; then
		$TMUX_PROGRAM move-window -s "${session_name}:${created_window_num}" -t "${session_name}:${window_number}"
	fi
}
...
new_pane() {
	...
	if is_restoring_pane_contents && pane_contents_file_exists "$pane_id"; then
		local pane_creation_command="$(pane_creation_command "$session_name" "$window_number" "$pane_index")"
		$TMUX_PROGRAM split-window -t "${session_name}:${window_number}" -c "$dir" "$pane_creation_command"
	else
		$TMUX_PROGRAM split-window -t "${session_name}:${window_number}" -c "$dir"
	fi
	# minimize window so more panes can fit
	$TMUX_PROGRAM resize-pane -t "${session_name}:${window_number}" -U "999"
}
	...
	while IFS=$d read line_type session_name window_number window_active window_flags pane_index pane_title dir pane_active pane_command pane_full_command; do
		...
		if pane_exists "$session_name" "$window_number" "$pane_index"; then
			if is_restoring_from_scratch; then
				...
				local pane_id="$($TMUX_PROGRAM display-message -p -F "#{pane_id}" -t "$session_name:$window_number")"
				new_pane "$session_name" "$window_number" "$dir" "$pane_index"
				$TMUX_PROGRAM kill-pane -t "$pane_id"
			else
		...
		# set pane title
		$TMUX_PROGRAM select-pane -t "$session_name:$window_number.$pane_index" -T "$pane_title"
	done < <(echo "$pane")
}
...
restore_state() {
	...
	while IFS=$d read line_type client_session client_last_session; do
		$TMUX_PROGRAM switch-client -t "$client_last_session"
		$TMUX_PROGRAM switch-client -t "$client_session"
	done
}
...
restore_grouped_session() {
	...
	while IFS=$d read line_type grouped_session original_session alternate_window active_window; do
		TMUX="" $TMUX_PROGRAM -S "$(tmux_socket)" new-session -d -s "$grouped_session" -t "$original_session"
	done
}
...
restore_active_and_alternate_windows_for_grouped_sessions() {
	...
	while IFS=$d read line_type grouped_session original_session alternate_window_index active_window_index; do
		...
		if [ -n "$alternate_window_index" ]; then
			$TMUX_PROGRAM switch-client -t "${grouped_session}:${alternate_window_index}"
		fi
		if [ -n "$active_window_index" ]; then
			$TMUX_PROGRAM switch-client -t "${grouped_session}:${active_window_index}"
		fi
	done
}
...
detect_if_restoring_from_scratch() {
	...
	local total_number_of_panes="$($TMUX_PROGRAM list-panes -a | wc -l | sed 's/ //g')"
...
handle_session_0() {
	if is_restoring_from_scratch && ! has_restored_session_0; then
		local current_session="$($TMUX_PROGRAM display -p "#{client_session}")"
		if [ "$current_session" == "0" ]; then
			$TMUX_PROGRAM switch-client -n
		fi
		$TMUX_PROGRAM kill-session -t "0"
	fi
}
...
restore_window_properties() {
	...
	\grep '^window' $(last_resurrect_file) |
		while IFS=$d read line_type session_name window_number window_name window_active window_flags window_layout automatic_rename; do
			$TMUX_PROGRAM select-layout -t "${session_name}:${window_number}" "$window_layout"
			...
			$TMUX_PROGRAM rename-window -t "${session_name}:${window_number}" "$window_name"
			if [ "${automatic_rename}" = ":" ]; then
				$TMUX_PROGRAM set-option -u -t "${session_name}:${window_number}" automatic-rename
			else
				$TMUX_PROGRAM set-option -t "${session_name}:${window_number}" automatic-rename "$automatic_rename"
			fi
		done
}
...
restore_active_pane_for_each_window() {
	awk 'BEGIN { FS="\t"; OFS="\t" } /^pane/ && $9 == 1 { print $2, $3, $6; }' $(last_resurrect_file) |
		while IFS=$d read session_name window_number active_pane; do
			$TMUX_PROGRAM switch-client -t "${session_name}:${window_number}"
			$TMUX_PROGRAM select-pane -t "$active_pane"
		done
}
...
restore_zoomed_windows() {
	awk 'BEGIN { FS="\t"; OFS="\t" } /^pane/ && $5 ~ /Z/ && $9 == 1 { print $2, $3; }' $(last_resurrect_file) |
		while IFS=$d read session_name window_number; do
			$TMUX_PROGRAM resize-pane -t "${session_name}:${window_number}" -Z
		done
}

...
dump_panes_raw() {
	$TMUX_PROGRAM list-panes -a -F "$(pane_format)"
}

dump_windows_raw(){
	$TMUX_PROGRAM list-windows -a -F "$(window_format)"
}

toggle_window_zoom() {
	local target="$1"
	$TMUX_PROGRAM resize-pane -Z -t "$target"
}
...
number_nonempty_lines_on_screen() {
	local pane_id="$1"
	$TMUX_PROGRAM capture-pane -pJ -t "$pane_id" |
		sed '/^$/d' |
		wc -l |
		sed 's/ //g'
}
...
pane_has_any_content() {
	local pane_id="$1"
	local history_size="$($TMUX_PROGRAM display -p -t "$pane_id" -F "#{history_size}")"
	local cursor_y="$($TMUX_PROGRAM display -p -t "$pane_id" -F "#{cursor_y}")"
...
capture_pane_contents() {
	...
	if pane_has_any_content "$pane_id"; then
		...
		printf '%s\n' "$($TMUX_PROGRAM capture-pane -epJ -S "$start_line" -t "$pane_id")" > "$(pane_contents_file "save" "$pane_id")"
	fi
}

get_active_window_index() {
	local session_name="$1"
	$TMUX_PROGRAM list-windows -t "$session_name" -F "#{window_flags} #{window_index}" |
		awk '$1 ~ /\*/ { print $2; }'
}
...
dump_grouped_sessions() {
	...
	$TMUX_PROGRAM list-sessions -F "$(grouped_sessions_format)" |
		grep "^1" |
		cut -c 3- |
		sort |
...
dump_windows() {
	dump_windows_raw |
		while IFS=$d read line_type session_name window_index window_name window_active window_flags window_layout; do
			...
			automatic_rename="$($TMUX_PROGRAM show-window-options -vt "${session_name}:${window_index}" automatic-rename)"
...
dump_state() {
	$TMUX_PROGRAM display-message -p "$(state_format)"
}
